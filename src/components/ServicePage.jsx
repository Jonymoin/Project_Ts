...
const Hero3 = () => {
  const services = [
    ...
    { name: "electrical service", slug: "electrical-service", icon: BiDish },
        ...
        <div className='grid grid-cols-2 md:grid-cols-4 gap-6 p-4'>
          {services.map(service => (
            <Link key={service.slug} to={`/service/${service.slug}`}>
              <div className='bg-[#1598FF] flex flex-col items-center py-10 text-black rounded-lg shadow-lg cursor-pointer group hover:bg-black transition-all duration-500'>
                <div className='bg-black p-4 rounded-full text-2xl group-hover:bg-[#1598FF]'>
                  <service.icon />
                </div>
